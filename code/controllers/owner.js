...
module.exports = {
  ...
  allotcook1_post: function (req, res) {
    ...
    Owner.insert_into_cooks(cook_id, cook_name, order_id, dish_id)
      .then(() => {
        setTimeout(function () {
          res.redirect("/owner");
        }, 1000);
  ...
  allotwaiter1_post: function (req, res) {
    ...
    Owner.insert_into_delivers(waiter_id, waiter_name, order_id, dish_id)
      .then(() => {
        setTimeout(function () {
          res.redirect("/owner");
        }, 1000);
    ...
    Promise.all(promiseList)
      .then((resp) => {
        res.render("./includes/owner", {
          ...
          empl: resp[1].rows,
          invs: resp[0].rows,
          orders_all: resp[2].rows,
          cooks_all: resp[3].rows,
          serves_all: resp[4].rows,
          users_all: resp[5].rows,
